...
const RESERVED = [
  'www','app','admin','api','dashboard','login','register','assets','static','support','help','blog','demo','pricing','account','settings'
];
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (validation) return res.status(400).json({ available: false, message: validation });

  if (RESERVED.includes(s)) return res.status(200).json({ available: false, message: 'Ten adres jest zarezerwowany' });
  ...
  try {
    ...
    if (data && data.length > 0) {
      return res.status(200).json({ available: false, message: 'Ten adres jest już zajęty' });
    }
